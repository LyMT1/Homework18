package com.onemount.data.repository;
import java.io.File;
...
public class PersonRepository {
  ...
  public Map<String,List<Person>> devhanoisaigoshanghai() {
    ...
    return groupDevelopersByCity.entrySet()
            .stream()
  ...
  public Map<String, Double> malefemaleratio() {
    ...
    Map<String, List<Person>> groupByCity = people
            .stream()
            .collect(Collectors.groupingBy(Person::getCity));
    for (Map.Entry<String, List<Person>> map : groupByCity.entrySet()) {
      long countMale = map.getValue().stream()
